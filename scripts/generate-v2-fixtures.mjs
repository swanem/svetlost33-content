...
const definitions = [
  ...
  { id:'backgrounds-core', type:'backgrounds', version:'1.0.0', required:false, capabilities:['background-catalog-v1'], dependencies:[], files:[['data/catalog.json',{items:[{id:'fixture-light-png',revision:1,file:'media/light.png',width:1,height:1,rights:'fixture-only'},{id:'fixture-light-jpeg',revision:1,file:'media/light.jpg',width:2,height:2,rights:'fixture-only'}]}],['media/light.png',onePixelPng,'image/png',{image:{width:1,height:1}}],['media/light.jpg',onePixelJpeg,'image/jpeg',{image:{width:2,height:2}}]] }
];
const moduleEntries = [];
for (const definition of definitions) {
  ...
  const moduleId = scenario === 'invalid-module-id' && definition.id === 'library-core' ? '../library' : definition.id;
  const version = scenario === 'invalid-version' && definition.id === 'library-core' ? '1 0' : definition.version;
  const capabilities = scenario === 'capability-non-string' && definition.id === 'library-core' ? [true] : definition.capabilities;
  const manifest = json({ schema_version:schemaVersion, module_id:moduleId, module_type:definition.type, version, revision:1, key_id:keyId, capabilities, dependencies:definition.dependencies, files:records });
  await put(`${dir}/manifest.json`, manifest); await put(`${dir}/manifest.sig`, signed(manifest, privateKey));
  moduleEntries.push({ module_id:moduleId, module_type:definition.type, version, required:scenario === 'required-string' && definition.id === 'library-core' ? 'true' : definition.required, manifest_path:`${dir}/manifest.json`, manifest_bytes:manifest.length, manifest_sha256:sha256(manifest), signature_path:`${dir}/manifest.sig` });
}
...
const releaseSet = json({ schema_version:'svetlost33-release-set-2', release_set_id:'fixture-release-2026-r1', sequence:1, channel:'production', key_id:keyId, approved_platforms:scenario === 'invalid-platforms' ? ['android','android'] : ['android','ios'], min_clients:{android:scenario === 'min-android-string' ? '10' : 10,ios:scenario === 'min-ios-invalid' ? '0.15' : '0.15.0'}, modules:moduleEntries });
await put('release-set.json', releaseSet); await put('release-set.sig', signed(releaseSet, privateKey));
const sequence = scenario === 'sequence-bool' ? true : scenario === 'sequence-fractional' ? 1.5 : 1;
const issuedAt = scenario === 'future-issued' ? '2099-01-01T00:00:00Z' : scenario === 'expired-index' ? '2019-09-15T00:00:00Z' : '2026-09-15T00:00:00Z';
const expiresAt = scenario === 'expired-index' ? '2020-09-16T00:00:00Z' : '2099-09-15T00:00:00Z';
const index = json({ schema_version:'svetlost33-m0-index-2', sequence, issued_at:issuedAt, expires_at:expiresAt, key_id:scenario === 'unknown-key' ? 'svetlost33-unknown-key-1' : keyId, channels:{production:{release_set_id:'fixture-release-2026-r1',path:'release-set.json',bytes:releaseSet.length,sha256:sha256(releaseSet),signature_path:'release-set.sig'}} });
...
if (scenario === 'garbage-signature') await put('index.sig', Buffer.from('@@not-base64@@\n'));
if (scenario === 'oversized-signature') await put('index.sig', Buffer.alloc(8193, 65));
